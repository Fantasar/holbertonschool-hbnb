...
class ReviewList(Resource):
    ...
    def post(self):
        ...
        if place.owner.id == current_user:  # compare avec current_user
            # Modifié le message d'erreur
            return {'error': 'You cannot review your own place'}, 400

        # empêche de reviewer le même place 2 fois
        existing_reviews = [
            r for r in place.reviews if r.user.id == current_user]
        if existing_reviews:
            return {'error': 'You have already reviewed this place'}, 400

        review_data['user_id'] = current_user  # force user_id = user connecté
